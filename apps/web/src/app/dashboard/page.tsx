...
const StatCard = dynamic(() => import('@/components/StatCard').then((m) => m.StatCard), { ssr: false });
...
export default function DashboardPage() {
  ...
  const [hwGameId, setHwGameId] = useState(games[0]?.id || 'game-1');
  ...
  const handleAddHomework = (e: React.FormEvent) => {
    ...
    setHomeworkList([
      ...homeworkList,
      {
        ...
        gameTitle: game?.title || 'Learning Game',
        gameId: game?.id || 'game-1',
        status: 'PENDING'
      }
    ]);
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `class-performance-${Date.now()}.csv`);
          ...
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            ...
            {role === 'TEACHER' && (
              <div className="flex gap-2">
                ...
                >
                  <IconPlus className="size-4" /> Assign Homework
                </button>
                <button
                ...
                >
                  <IconDownload className="size-4" /> Export Class Report
                </button>
              </div>
            )}
          </div>
          ...
          <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                <StatCard label="Games Played" value="34" icon={IconGamepad} tone="bg-emerald-100 text-emerald-700" />
                <StatCard label="Stars" value="86" icon={IconStar} tone="bg-yellow-100 text-yellow-700" />
                <StatCard label="Badges" value="7" icon={IconAward} tone="bg-pink-100 text-pink-700" />
              </div>
            </div>
              ...
              {homeworkList.map((hw) => (
                <div key={hw.id} className="mt-3 border-t border-slate-700/60 pt-3">
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    >
                      <IconPlay className="size-4" /> Launch Game
                    </Link>
                  </div>
                </div>
              ))}
          ...
          <section className="grid gap-6 lg:grid-cols-3">
            <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 lg:col-span-2">
              <div className="flex items-center justify-between">
                ...
                <IconBarChart className="size-6 text-emerald-700" />
              </div>
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <StatCard label="Assigned Students" value="31" icon={IconUsers} tone="bg-sky-100 text-sky-700" />
                <StatCard label="Homework Completed" value="82%" icon={IconBookCheck} tone="bg-lime-100 text-lime-700" />
                <StatCard label="Average Score" value="78%" icon={IconBarChart} tone="bg-violet-100 text-violet-700" />
              </div>
              ...
              <div className="mt-6 rounded-xl bg-rose-50 p-4 border border-rose-100">
                <div className="flex items-center gap-2 text-rose-900 font-black">
                  <IconAlertTriangle className="size-5" /> Students Needing Attention
                </div>
